...
weekPost <- CD8cdr3PostProcess$weekPost
...
geom_bar(aes(x = 3, temp2.mod$V2, fill = as.factor(temp2.mod$fillseq), alpha = as.factor(temp2.mod$shared)), stat = 'identity', colour = 'black', size = 0.1) +
  ...
  geom_errorbar(aes(x = 5, ymin = round(minPcntShared-yadjst), ymax = round(minPcntShared-yadjst)))+
          ...
          labs(title = expression(paste('Shared Clonotypes Patient T CD8+ ',alpha, ' chain')), x = 'Week', y = 'Cumulative % of Sample')+
...
for (i in unique(shared.mod$ID)){
  for (j in unique(shared.mod$chain)){
    ...
    ggp <- ggplot()+
      ...
      labs(x = 'Time (weeks)', y = 'Sample of interest (weeks)', nudge_y = 0.9,title = paste('PATIENT', 'T','CD8', 'alpha chain'))+
      theme_classic()+
...
write.csv(shared, "shared_TCD8a.csv")
