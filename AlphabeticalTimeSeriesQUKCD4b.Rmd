...
geom_errorbar(aes(x = 7, ymin = round(minPcntShared-yadjst), ymax = round(minPcntShared-yadjst)))+
          ...
          labs(title = expression(paste('Shared Clonotypes Patient K CD4+ ',beta, ' chain')), x = 'Week', y = 'Cumulative % of Sample')+
...
for (i in unique(shared.mod$ID)){
  for (j in unique(shared.mod$chain)){
    ...
    ggp <- ggplot()+
      ...
      labs(x = 'Time (weeks)', y = 'Sample of interest (weeks)', nudge_y = 0.09,title = paste('PATIENT', 'K','CD4', 'beta chain'))+
      theme_classic()+
...
write.csv(shared, "shared_KCD4b.csv")
