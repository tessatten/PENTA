...
weekPosts <- unique(dat$weekPost[dat$ID == "L" & dat$chain == "a"]) # selects unique time points
...
firstDataFrame = subset(dat, subset=(dat$ID == "L" & dat$chain == "a")) #creates a dataframe with just the data of interest - eg Patient K, alpha chain, CD8 cells

tempor = read.csv("dcr_alpha_QULPTIW12-a.cdr3_2.csv", stringsAsFactors = F, header = F)
colnames(tempor)[1] <- 'CDR3_AA_sequence'
colnames(tempor)[2] <- 'Frequency'
tempor$ID = "QULPTIW"
tempor$weekPost = 12
tempor$chain = "a"
tempor$sampleID = "QULPTIW12-a"
tempor$antigen = NA
tempor$score = NA
firstDataFrame= rbind(QULCD4alphaW0, tempor, QULCD4alphaW150)
newQULCD4alpha= rbind(QULCD4alphaW0, tempor, QULCD4alphaW48, QULCD4alphaW150)

firstDataFrame$ID[firstDataFrame$ID == 'QULPTIW'] <- 'L'
...
temp1 <- datWithFillseq[datWithFillseq$ID == "L" & datWithFillseq$weekPost == "0" & datWithFillseq$chain == "a",] #t0 first time point
temp2 <- datWithFillseq[datWithFillseq$ID == "L" & datWithFillseq$weekPost == "12" & datWithFillseq$chain == "a",] #t1 second time point
temp4 <- datWithFillseq[datWithFillseq$ID == "L" & datWithFillseq$weekPost == "150" & datWithFillseq$chain == "a",] #fourth time point
...
geom_bar(aes(x = 3, temp2.mod$V2, fill = as.factor(temp2.mod$fillseq), alpha = as.factor(temp2.mod$shared)), stat = 'identity', colour = 'black', size = 0.1) +
  ...
  geom_errorbar(aes(x = 5, ymin = round(minPcntShared-yadjst), ymax = round(minPcntShared-yadjst)))+
          ...
          scale_x_continuous(breaks = c(1, 3, 5), labels = c(150, 12, 0))+
          ...
          labs(title = expression(paste('Shared Clonotypes Patient L CD4+ ',alpha, ' chain')), x = 'Week', y = 'Cumulative % of Sample')+
...
for (i in unique(shared.mod$ID)){
  for (j in unique(shared.mod$chain)){
    ...
    ggp2 <- ggplot()+
      ...
      labs(x = 'Time (weeks)', y = 'Sample of interest (weeks)', nudge_y = 0.9,title = paste('PATIENT', 'L','CD4', 'alpha chain'))+
      theme_classic()+
    ...
    ggsave(paste(paste(i, j, sep = '_'), '.pdf', sep = ''), 
           ggp2, width = 20, height = 25, units = c('cm'))
    
  }
}
#write.csv(shared, "shared_LCD4a.csv")
