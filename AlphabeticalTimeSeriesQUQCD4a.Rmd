...
geom_errorbar(aes(x = 7, ymin = round(minPcntShared-yadjst), ymax = round(minPcntShared-yadjst)))+
          ...
          labs(title = expression(paste('Shared Clonotypes Patient Q CD4+ ',alpha, ' chain')), x = 'Week', y = 'Cumulative % of Sample')+
...
for (i in unique(shared.mod$ID)){
  for (j in unique(shared.mod$chain)){
    ...
    ggp <- ggplot()+
      ...
      labs(x = 'Time (weeks)', y = 'Sample of interest (weeks)', nudge_y = 0.09,title = paste('PATIENT', 'Q','CD4', 'alpha chain'))+
      theme_classic()+
...
write.csv(shared, "shared_QCD4a.csv")
