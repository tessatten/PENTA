...
ggsave(paste(wd, '/', paste(sampleName1, sampleName2, sep = '_'), '_CD4_PTI_final',  '.pdf', sep = ''), 
...
for (i in unique(shared.mod$ID)){
  for (j in unique(shared.mod$chain)){
    ...
    ggp <- ggplot()+
      ...
      labs(x = 'Time (weeks)', y = 'Sample of interest (weeks)', nudge_y = 0.09,title = paste('PATIENT', 'Q','CD4', 'beta chain'))+
      theme_classic()+
...
write.csv(shared, "shared_QCD4b.csv")
