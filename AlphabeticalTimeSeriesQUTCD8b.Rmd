...
temp3 <- datWithFillseq[datWithFillseq$ID == "T" & datWithFillseq$weekPost == "48" & datWithFillseq$chain == "b",] #third time point
temp4 <- datWithFillseq[datWithFillseq$ID == "T" & datWithFillseq$weekPost == "150" & datWithFillseq$chain == "b",] #fourth time point
...
geom_errorbar(aes(x = 7, ymin = round(minPcntShared-yadjst), ymax = round(minPcntShared-yadjst)))+
          ...
          labs(title = expression(paste('Shared Clonotypes Patient T CD8+ ',beta, ' chain')), x = 'Week', y = 'Cumulative % of Sample')+
...
for (i in unique(shared.mod$ID)){
  for (j in unique(shared.mod$chain)){
    ...
    ggp <- ggplot()+
      ...
      labs(x = 'Time (weeks)', y = 'Sample of interest (weeks)', nudge_y = 0.09,title = paste('PATIENT', 'T','CD8', 'beta chain'))+
      theme_classic()+
...
write.csv(shared, "shared_TCD8b.csv")
