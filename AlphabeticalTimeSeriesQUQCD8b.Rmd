...
weekPosts <- unique(dat$weekPost[dat$ID == "Q" & dat$chain == "b"]) # selects unique time points
...
firstDataFrame = subset(dat, subset=(dat$ID == "Q" & dat$chain == "b")) #creates a dataframe with just the data of interest - eg Patient K, alpha chain, CD8 cells
...
temp1 <- datWithFillseq[datWithFillseq$ID == "Q" & datWithFillseq$weekPost == "0" & datWithFillseq$chain == "b",] #t0 first time point
temp2 <- datWithFillseq[datWithFillseq$ID == "Q" & datWithFillseq$weekPost == "12" & datWithFillseq$chain == "b",] #t1 second time point
temp3 <- datWithFillseq[datWithFillseq$ID == "Q" & datWithFillseq$weekPost == "48" & datWithFillseq$chain == "b",] #third time point
temp4 <- datWithFillseq[datWithFillseq$ID == "Q" & datWithFillseq$weekPost == "150" & datWithFillseq$chain == "b",] #fourth time point
...
geom_errorbar(aes(x = 7, ymin = round(minPcntShared-yadjst), ymax = round(minPcntShared-yadjst)))+
          ...
          labs(title = expression(paste('Shared Clonotypes Patient Q CD8+ ',beta, ' chain')), x = 'Week', y = 'Cumulative % of Sample')+
...
for (i in unique(shared.mod$ID)){
  for (j in unique(shared.mod$chain)){
    ...
    ggp <- ggplot()+
      ...
      labs(x = 'Time (weeks)', y = 'Sample of interest (weeks)', nudge_y = 0.09,title = paste('PATIENT', 'Q','CD8', 'beta chain'))+
      theme_classic()+
...
write.csv(shared, "shared_QCD8b.csv")
