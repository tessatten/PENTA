...
ID <- CD8cdr3PostProcess$ID
weekPost <- CD8cdr3PostProcess$week
chain <- CD8cdr3PostProcess$chain
V1 <- CD8cdr3PostProcess$V1 #CDR3s
V2 <- CD8cdr3PostProcess$V2 #Freqs
...
weekPosts <- unique(dat$weekPost[dat$ID == "V" & dat$chain == "b"]) # selects unique time points
...
firstDataFrame = subset(dat, subset=(dat$ID == "V" & dat$chain == "b")) #creates a dataframe with just the data of interest - eg Patient K, alpha chain, CD8 cells
...
temp1 <- datWithFillseq[datWithFillseq$ID == "V" & datWithFillseq$weekPost == "0" & datWithFillseq$chain == "b",] #t0 first time point
temp2 <- datWithFillseq[datWithFillseq$ID == "V" & datWithFillseq$weekPost == "48" & datWithFillseq$chain == "b",] #t1 second time point
temp4 <- datWithFillseq[datWithFillseq$ID == "V" & datWithFillseq$weekPost == "150" & datWithFillseq$chain == "b",] #fourth time point
...
geom_bar(aes(x = 3, temp2.mod$V2, fill = as.factor(temp2.mod$fillseq), alpha = as.factor(temp2.mod$shared)), stat = 'identity', colour = 'black', size = 0.1) +
  ...
  geom_errorbar(aes(x = 5, ymin = round(minPcntShared-yadjst), ymax = round(minPcntShared-yadjst)))+
          ...
          scale_x_continuous(breaks = c(1, 3, 5), labels = c(sampleName4, sampleName2, sampleName1))+
...
ggsave(paste(paste(sampleName1, sampleName2, sampleName4, sep = '_'), '_CD8_CT_final',  '.pdf', sep = ''), 
...
for (i in unique(shared.mod$ID)){
  for (j in unique(shared.mod$chain)){
    ...
    ggp <- ggplot()+
      ...
      labs(x = 'Time (weeks)', y = 'Sample of interest (weeks)', nudge_y = 0.9,title = paste('PATIENT', 'V','CD8', 'beta chain'))+
      theme_classic()+
...
write.csv(shared, "shared_VCD8b.csv")
