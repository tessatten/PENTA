...
weekPosts <- unique(dat$weekPost[dat$ID == "E" & dat$chain == "b"]) # selects unique time points
...
firstDataFrame = subset(dat, subset=(dat$ID == "E" & dat$chain == "b")) #creates a dataframe with just the data of interest - eg Patient K, alpha chain, CD8 cells
...
temp1 <- datWithFillseq[datWithFillseq$ID == "E" & datWithFillseq$weekPost == "0" & datWithFillseq$chain == "b",] #t0 first time point
temp2 <- datWithFillseq[datWithFillseq$ID == "E" & datWithFillseq$weekPost == "150" & datWithFillseq$chain == "b",] #t1 second time point
...
ggsave(paste(paste(sampleName1, sampleName2, sep = '_'), '_CD8_CT_final',  '.pdf', sep = ''), 
...
write.csv(shared, "shared_ECD8b.csv")
...
for (i in unique(shared.mod$ID)){
  for (j in unique(shared.mod$chain)){
    ...
    ggp <- ggplot()+
      ...
      labs(x = 'Time (weeks)', y = 'Sample of interest (weeks)', nudge_y = 0.09,title = paste('PATIENT', 'E','CD8', 'beta chain'))+
      theme_classic()+
