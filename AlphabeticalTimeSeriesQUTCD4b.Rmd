...
weekPosts <- unique(dat$weekPost[dat$ID == "T" & dat$chain == "b"]) # selects unique time points
...
firstDataFrame = subset(dat, subset=(dat$ID == "T" & dat$chain == "b")) #creates a dataframe with just the data of interest - eg Patient K, alpha chain, CD8 cells

tempor = read.csv("dcr_beta_QUTPTIW12-b.cdr3_2.csv", stringsAsFactors = F, header = F)
colnames(tempor)[1] <- 'CDR3_AA_sequence'
colnames(tempor)[2] <- 'Frequency'
tempor$ID = "QUTPTIW"
tempor$weekPost = 12
tempor$chain = "b"
tempor$sampleID = "QUTPTIW12-b"
tempor$antigen = NA
tempor$score = NA
firstDataFrame= rbind(QUTCD4betaW0, tempor, QUTCD4betaW48)
QUTCD4beta= rbind(QUTCD4betaW0, tempor, QUTCD4betaW48, QUTCD4betaW150)
...
temp1 <- datWithFillseq[datWithFillseq$ID == "T" & datWithFillseq$weekPost == "0" & datWithFillseq$chain == "b",] #t0 first time point
temp2 <- datWithFillseq[datWithFillseq$ID == "T" & datWithFillseq$weekPost == "12" & datWithFillseq$chain == "b",] #t1 second time point
temp4 <- datWithFillseq[datWithFillseq$ID == "T" & datWithFillseq$weekPost == "48" & datWithFillseq$chain == "b",] #fourth time point
...
temp1$shared[temp1$V1 %in% temp2$V1 | temp1$V1 %in% temp4$V1] <- T # marks clonotypes that are in temp1 and also in temp2 OR temp 3 OR temp4 as TRUE
temp1$shared[!temp1$V1 %in% temp2$V1 &  !temp1$V1 %in% temp4$V1] <- F # marks clonotypes that are in temp1 and NOT in temp2 OR temp3 OR temp4 as FALSE

temp2$shared[temp2$V1 %in% temp1$V1 | temp2$V1 %in% temp4$V1] <- T  # marks clonotypes that are in temp2 and temp1 as TRUE
temp2$shared[!temp2$V1 %in% temp1$V1  & !temp2$V1 %in% temp4$V1] <- F#  marks clonotypes that are in temp2 and NOT in temp1 as FALSE


temp4$shared[temp4$V1 %in% temp1$V1 | temp4$V1 %in% temp2$V1 ] <- T  # marks clonotypes that are in temp2 and temp1 as TRUE
temp4$shared[!temp4$V1 %in% temp1$V1 & !temp4$V1 %in% temp2$V1 ] <- F#  marks clonotypes that are in temp2 and NOT in temp1 as FALSE
...
maxPcntShared <- max(c(100 - sum(temp1.mod$V2[temp1.mod$shared==T]), 100 - sum(temp2.mod$V2[temp2.mod$shared==T]),  100 - sum(temp4.mod$V2[temp4.mod$shared==T]))) #calculates highest % of uniques

minPcntShared <- min(c(100 - sum(temp1.mod$V2[temp1.mod$shared==T]), 100 - sum(temp2.mod$V2[temp2.mod$shared==T]), 100 - sum(temp4.mod$V2[temp4.mod$shared==T]))) #calculates lowest % of uniques (gives % of reads that come from clonotypes unique to the sample)
...
temp2.mod$x <- 2.55 #new column all equalling 2.55
temp4.mod$x <- 1.45 #new column all equalling 3.65
...
intersectB1 <- temp2.mod[temp2.mod$V1 %in% temp4.mod$V1,] # overlap overlap( finds all the seqs in temp1 that also appear in temp 2)
intersectB2 <- temp4.mod[temp4.mod$V1 %in% temp2.mod$V1,] # overlap overlap( finds all the seqs in temp1 that also appear in temp 2)
...
geom_bar(aes(x = 3, temp2.mod$V2, fill = as.factor(temp2.mod$fillseq), alpha = as.factor(temp2.mod$shared)), stat = 'identity', colour = 'black', size = 0.1) +
  geom_bar(aes(x = 5, temp1.mod$V2, fill = as.factor(temp1.mod$fillseq), alpha = as.factor(temp1.mod$shared)), stat = 'identity', colour = 'black', size = 0.1) + 
  geom_errorbar(aes(x = 5, ymin = round(minPcntShared-yadjst), ymax = round(minPcntShared-yadjst)))+
          ...
          scale_x_continuous(breaks = c(1, 3, 5), labels = c(48, 12, 0))+
          ...
          #annotate(geom = "segment", x = -Inf, xend = -Inf, y =  round(minPcntShared+4), yend = round(minPcntShared, -6),
           #        linetype = "dashed", color = "white")+
          coord_flip(ylim=c(round(minPcntShared-yadjst), 100), expand = T) +
          labs(title = expression(paste('Shared Clonotypes Patient T CD4+ ',beta, ' chain')), x = 'Week', y = 'Cumulative % of Sample')+
          theme(legend.position="none", panel.background = element_blank())
        
ggsave(paste(wd, '/', paste(sampleName1, sampleName2, sampleName4, sep = '_'), '_CD4_PTI_final',  '.pdf', sep = ''), 
...
for (i in unique(shared.mod$ID)){
  for (j in unique(shared.mod$chain)){
    ...
    ggp <- ggplot()+
      ...
      geom_text(aes(x = unique(tempx), y = as.factor(unique(c(temp$x1, temp$x2))), label = '|'), nudge_y = 0.05)+
      ...
      labs(x = 'Time (weeks)', y = 'Sample of interest (weeks)', nudge_y = 0.9,title = paste('PATIENT', 'T','CD4', 'beta chain'))+
      theme_classic()+
...
write.csv(shared, "shared_TCD4b.csv")
