...
weekPosts <- unique(dat$weekPost[dat$ID == "L" & dat$chain == "b"]) # selects unique time points
...
firstDataFrame = subset(dat, subset=(dat$ID == "L" & dat$chain == "b")) #creates a dataframe with just the data of interest - eg Patient K, alpha chain, CD8 cells

tempor = read.csv("dcr_beta_QULPTIW12-b.cdr3_2.csv", stringsAsFactors = F, header = F)
colnames(tempor)[1] <- 'CDR3_AA_sequence'
colnames(tempor)[2] <- 'Frequency'
tempor$ID = "QULPTIW"
tempor$weekPost = 12
tempor$chain = "b"
tempor$sampleID = "QULPTIW12-b"
tempor$antigen = NA
tempor$score = NA
firstDataFrame= rbind(QULCD4betaW0, tempor, QULCD4betaW48, QULCD4betaW150)
QULCD4beta= rbind(QULCD4betaW0, tempor, QULCD4betaW48, QULCD4betaW150)
...
temp1 <- datWithFillseq[datWithFillseq$ID == "L" & datWithFillseq$weekPost == "0" & datWithFillseq$chain == "b",] #t0 first time point
temp2 <- datWithFillseq[datWithFillseq$ID == "L" & datWithFillseq$weekPost == "12" & datWithFillseq$chain == "b",] #t1 second time point
temp3 <- datWithFillseq[datWithFillseq$ID == "L" & datWithFillseq$weekPost == "48" & datWithFillseq$chain == "b",] #third time point
temp4 <- datWithFillseq[datWithFillseq$ID == "L" & datWithFillseq$weekPost == "150" & datWithFillseq$chain == "b",] #fourth time point
...
geom_errorbar(aes(x = 7, ymin = round(minPcntShared-yadjst), ymax = round(minPcntShared-yadjst)))+
          ...
          labs(title = expression(paste('Shared Clonotypes Patient L CD4+ ',beta, ' chain')), x = 'Week', y = 'Cumulative % of Sample')+
...
for (i in unique(shared.mod$ID)){
  for (j in unique(shared.mod$chain)){
    ...
    ggp <- ggplot()+
      ...
      labs(x = 'Time (weeks)', y = 'Sample of interest (weeks)', nudge_y = 0.09,title = paste('PATIENT', 'L','CD4', 'beta chain'))+
      theme_classic()+
    ...
    ggsave(paste(paste(i, j, sep = '_'), '.pdf', sep = ''), 
           ggp, width = 21, height = 25, units = c('cm'))
    
  }
}
write.csv(shared, "shared_LCD4b.csv")
