...
weekPosts <- unique(dat$weekPost[dat$ID == "V" & dat$chain == "a"]) # selects unique time points
...
firstDataFrame = subset(dat, subset=(dat$ID == "V" & dat$chain == "a")) #creates a dataframe with just the data of interest - eg Patient K, alpha chain, CD8 cells
...
temp1 <- datWithFillseq[datWithFillseq$ID == "V" & datWithFillseq$weekPost == "0" & datWithFillseq$chain == "a",] #t0 first time point
temp2 <- datWithFillseq[datWithFillseq$ID == "V" & datWithFillseq$weekPost == "48" & datWithFillseq$chain == "a",] #t1 second time point
temp4 <- datWithFillseq[datWithFillseq$ID == "V" & datWithFillseq$weekPost == "150" & datWithFillseq$chain == "a",] #fourth time point
...
for (i in unique(shared.mod$ID)){
  for (j in unique(shared.mod$chain)){
    ...
    ggp <- ggplot()+
      ...
      labs(x = 'Time (weeks)', y = 'Sample of interest (weeks)', nudge_y = 0.9,title = paste('PATIENT', 'V','CD8', 'alpha chain'))+
      theme_classic()+
...
write.csv(shared, "shared_VCD8a.csv")
