...
weekPosts <- unique(dat$weekPost[dat$ID == "K" & dat$chain == "b"]) # selects unique time points
...
firstDataFrame = subset(dat, subset=(dat$ID == "K" & dat$chain == "b")) #creates a dataframe with just the data of interest - eg Patient K, alpha chain, CD8 cells
...
temp1 <- datWithFillseq[datWithFillseq$ID == "K" & datWithFillseq$weekPost == "0" & datWithFillseq$chain == "b",] #t0 first time point
temp2 <- datWithFillseq[datWithFillseq$ID == "K" & datWithFillseq$weekPost == "12" & datWithFillseq$chain == "b",] #t1 second time point
temp3 <- datWithFillseq[datWithFillseq$ID == "K" & datWithFillseq$weekPost == "48" & datWithFillseq$chain == "b",] #third time point
temp4 <- datWithFillseq[datWithFillseq$ID == "K" & datWithFillseq$weekPost == "150" & datWithFillseq$chain == "b",] #fourth time point
...
geom_errorbar(aes(x = 7, ymin = round(minPcntShared-yadjst), ymax = round(minPcntShared-yadjst)))+
          ...
          labs(title = expression(paste('Shared Clonotypes Patient K CD8+ ',beta, ' chain')), x = 'Week', y = 'Cumulative % of Sample')+
          theme(legend.position="none", panel.background = element_blank())
        
ggsave(paste(wd, '/', paste(sampleName1, sampleName2, sampleName3, sampleName4, sep = '_'), '_CD8_PTI_final',  '.pdf', sep = ''), 
...
for (i in unique(shared.mod$ID)){
  for (j in unique(shared.mod$chain)){
    ...
    ggp <- ggplot()+
      ...
      labs(x = 'Time (weeks)', y = 'Sample of interest (weeks)', nudge_y = 0.09,title = paste('PATIENT', 'K','CD8', 'beta chain'))+
      theme_classic()+
...
write.csv(shared, "shared_KCD8b.csv")
