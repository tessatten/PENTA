...
weekPosts <- unique(dat$weekPost[dat$ID == "F" & dat$chain == "a"]) # selects unique time points
...
firstDataFrame = subset(dat, subset=(dat$ID == "F" & dat$chain == "a")) #creates a dataframe with just the data of interest - eg Patient K, alpha chain, CD8 cells
...
temp1 <- datWithFillseq[datWithFillseq$ID == "F" & datWithFillseq$weekPost == "0" & datWithFillseq$chain == "a",] #t0 first time point
temp2 <- datWithFillseq[datWithFillseq$ID == "F" & datWithFillseq$weekPost == "150" & datWithFillseq$chain == "a",] #t1 second time point
...
write.csv(shared, "shared_FCD8a.csv")
...
for (i in unique(shared.mod$ID)){
  for (j in unique(shared.mod$chain)){
    ...
    ggp <- ggplot()+
      ...
      labs(x = 'Time (weeks)', y = 'Sample of interest (weeks)', nudge_y = 0.09,title = paste('PATIENT', 'F','CD8', 'alpha chain'))+
      theme_classic()+
