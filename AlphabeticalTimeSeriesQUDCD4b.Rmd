...
dat$ID[dat$ID == 'QUDCTW'] <- 'E'
dat$ID[dat$ID == 'QUDCTW'] <- 'F'
dat$ID[dat$ID == 'QUDCTW'] <- 'P'
dat$ID[dat$ID == 'QUDCTW'] <- 'V'
...
weekPosts <- unique(dat$weekPost[dat$ID == "D" & dat$chain == "b"]) # selects unique time points
...
firstDataFrame = subset(dat, subset=(dat$ID == "D" & dat$chain == "b")) #creates a dataframe with just the data of interest - eg Patient K, alpha chain, CD8 cells
...
temp1 <- datWithFillseq[datWithFillseq$ID == "D" & datWithFillseq$weekPost == "0" & datWithFillseq$chain == "b",] #t0 first time point
temp2 <- datWithFillseq[datWithFillseq$ID == "D" & datWithFillseq$weekPost == "150" & datWithFillseq$chain == "b",] #t1 second time point
...
for (i in unique(shared.mod$ID)){
  for (j in unique(shared.mod$chain)){
    ...
    ggp <- ggplot()+
      ...
      labs(x = 'Time (weeks)', y = 'Sample of interest (weeks)', nudge_y = 0.09,title = paste('PATIENT', 'D','CD4', 'beta chain'))+
      theme_classic()+
...
write.csv(shared, "shared_DCD4b.csv")
